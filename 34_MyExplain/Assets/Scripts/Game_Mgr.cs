...
#region --- Flexible  Joystick 처리부분
    ...
    void InvenScOnOffUpdate()
    {
        ...
        if (m_Inven_ScIsOn == false)
        {
            ...
            {
                // 인벤토리 스크롤의 위치를 m_ScOffPos 방향으로 m_ScSpeed * Time.deltaTime만큼 이동시킵니다.
        ...
        // 인벤토리 스크롤이 켜져있다면
        else if (m_Inven_ScIsOn == true)
        {
            
            if (m_InvenScrollTr.localPosition.x > m_ScOnPos.x)
            {
                // 인벤토리 스크롤의 위치를 m_ScOnPos 방향으로 m_ScSpeed * Time.deltaTime만큼 이동시킵니다.
                m_InvenScrollTr.localPosition = Vector3.MoveTowards(m_InvenScrollTr.localPosition,
